...
class AdagradOptimizer(optimizer):
    ...
    def update(self, model):
        ...
        for i, p in enumerate(model.parameters()):
            ## TODO
            # update r
            self.r[i] += p.grad * p.grad
            # update the direction of gradient
            p.grad = (self.learning_rate/(self.delta + self.r[i] ** 0.5)) * p.grad
...
class AdamOptimizer(optimizer):
    ...
    def update(self, model):
        ...
        for i, p in enumerate(model.parameters()):
            ...
            # correct bias
            m1_h = self.m1[i] / (1- self.beta1 ** self.iteration)
            m2_h = self.m2[i] / (1 - self.beta2 ** self.iteration)
            # update the direction of gradient
            p.grad = (self.learning_rate * m1_h) / (self.delta + m2_h ** 0.5)
